...
void OpenGLProcedures::LoadAllProcedures(irr::video::IContextManager *cmgr)
{
	...
	if (!GetBooleani_v) GetBooleani_v = (PFNGLGETBOOLEANI_VPROC_MT)cmgr->getProcAddress("glGetBooleani_v");
	...
	if (!GetInteger64i_v) GetInteger64i_v = (PFNGLGETINTEGER64I_VPROC_MT)cmgr->getProcAddress("glGetInteger64i_v");
	...
	if (!BlendBarrier) BlendBarrier = (PFNGLBLENDBARRIERPROC_MT)cmgr->getProcAddress("glBlendBarrier");
	if (!PrimitiveBoundingBox) PrimitiveBoundingBox = (PFNGLPRIMITIVEBOUNDINGBOXPROC_MT)cmgr->getProcAddress("glPrimitiveBoundingBox");
