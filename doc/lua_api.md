...
* `core.features`: Table containing *server-side* API feature flags
  ...
  ```lua
  {
      ...
      -- minetest.clear_objects function supports Lua callback. (5.15.0)
...
* `core.clear_objects([options])`
    ...
    * Takes an optional table as an argument with the fields:
        * `mode`
        ...
        * `callback`: Optional Lua callback function `function(name, staticdata, params`.
            * `name`: name of entity, `_builtin:item"` for example.
            * `staticdata`: test string, some asi is generated by `get_staticdata`
              and parsed to `on_activate` Lua entities callbacks.
